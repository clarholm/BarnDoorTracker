...
static const int pinOutStep = 2;      // Arduino digital pin connected to EasyDriver step
//#############UPDATE###############
static const int pinOutDirection = 3; // Arduino digital pin connected to EasyDriver direction
//#############UPDATE###############

static const int pinInSidereal = 8;  // Arduino analogue pin connected to sidereal mode switch
//#############UPDATE###############

static const int pinInHighspeed = 9; // Arduino analogue pin connected to highspeed mode switch
//#############UPDATE###############

static const int pinInDirection = 7; // Arduino analogue pin connected to direction

static const int motorOnOff = 10;
switch//#############UPDATE###############
...
void setup(void)
{
    ...
    pinMode(motorOnOff, INPUT_PULLUP);
...
void loop(void)
{
    ...
    if (digitalRead(pinInSidereal) == LOW && digitalRead(motorOnOff)==LOW) {
        barndoor.transitionTo(stateSidereal);
    } else if (digitalRead(pinInHighspeed) == LOW && digitalRead(motorOnOff)==LOW) {
